...
CSV_PATH  = "ch1.csv"
TARGET    = "Fuse at Mason Square CH-01 chw temp diff (ent - lvg)"
TIMESTAMP = "Timestamp"
...
LGBM_FIXED = dict(objective="regression", random_state=SEED, verbose=-1)
...
def tune(X, y):
    ...
    search = RandomizedSearchCV(
        LGBMRegressor(n_jobs=1, **LGBM_FIXED),
        space,
        n_iter=N_ITER,
...
def walk_forward(X, y, params):
    ...
    for train_idx, test_idx in TimeSeriesSplit(n_splits=N_SPLITS).split(X):
        model = LGBMRegressor(n_jobs=-1, **LGBM_FIXED, **params)
...
def regression_report(y_true, y_pred):
    ...
    print("\nHow well do we predict temperature?")
    print(f"  MAE  : {mae:.3f}")
...
def incident_report(y_true, y_pred, buffer=0.0):
    ...
    print(f"\nHow well do we predict incidents?  (buffer = {buffer:.2f})")
...
def threshold_sweep(y_true, y_pred):
    ...
    for buf in np.arange(0.0, 1.55, 0.1):
...
def main():
    ...
    print(f"{len(X):,} rows, {X.shape[1]} features, "
          f"{incidents} incidents ({incidents / len(y):.1%})")
